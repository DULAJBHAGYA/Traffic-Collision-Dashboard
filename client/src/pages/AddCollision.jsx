...
const AddCollision = ({ onSave, onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/collisions', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setModalMessage('Collision record created successfully!');
        setShowModal(true);
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          setModalMessage(errorData.message || `Failed to save record: ${response.status} ${response.statusText}`);
        }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mx-auto" style={{ maxWidth: '1200px' }}>
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Fill in the details for the statistics record. All fields marked with * are required.
            </p>
          </div>
